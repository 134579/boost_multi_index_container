...
struct tsc_chrono {
  ...
  static void init() { detail::init(); }

  void start() { m_start = detail::rdtsc(); }

  void restart() { start(); }

  int64_t elapsed() const { return detail::rdtsc() - m_start; }
  ...
  static std::chrono::nanoseconds from_cycles(int64_t cycles) {
    return std::chrono::nanoseconds(
        std::llround(cycles / detail::tsc::get_freq_ghz()));
  }

  template <typename _DurationT> static int64_t to_cycles(_DurationT duration) {
    return std::chrono::duration_cast<std::chrono::nanoseconds>(duration)
               .count() *
           detail::tsc::get_freq_ghz();
  }
