...
struct malloc_chrono {
  malloc_chrono() { tsc_chrono::init(); }

  void pre_malloc(size_t) { _chrono.start(); }

  void post_malloc(size_t, const void *) {
    _elapsed_time_malloc += _chrono.elapsed();
  }

  void pre_free(const void *) { _chrono.start(); }

  void post_free(const void *) { _elapsed_time_free += _chrono.elapsed(); }

  void pre_realloc(const void *, size_t) { _chrono.start(); }
