...
template <typename... Handlers> struct mtrace {
  mtrace() {
    save_hooks();
    load_custom_hooks();
  }

  ~mtrace() { restore_hooks(); }

  template <std::size_t I> auto &get() { return std::get<I>(_handlers); }
  ...
  static void *malloc(size_t size, const void *caller) {
    ...
    void *p = ::malloc(size);
    detail::for_each_in_tuple(_handlers,
                              [&](auto &x) { x.post_malloc(size, p); });
  ...
  static void *realloc(void *mem, size_t size, const void *caller) {
    ...
    detail::for_each_in_tuple(_handlers,
                              [&](auto &x) { x.pre_realloc(mem, size); });
    void *p = ::realloc(mem, size);
    detail::for_each_in_tuple(_handlers,
                              [&](auto &x) { x.post_realloc(mem, size, p); });
...
std::tuple<Handlers...> mtrace<Handlers...>::_handlers;
