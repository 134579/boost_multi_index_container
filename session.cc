...
struct session {
  ...
  session(const std::string &user, const std::string &script)
      : user_name(user), script_name(script) {}

  std::string id() const {
    return user_name.substr(0, 3) + script_name.substr(0, 3);
  }

  std::experimental::string_view user_name_view() const { return user_name; }
  std::experimental::string_view script_name_view() const {
    return script_name;
  }
...
void simple_index() {
  boost::multi_index_container<
      ...
      indexed_by<hashed_unique<
          tag<by_name>, member<session, std::string, &session::user_name>>>>
      sessions;
...
void composed_index() {
  boost::multi_index_container<
      session,
      indexed_by<hashed_unique<
          composite_key<session,
                        const_mem_fun<session, std::experimental::string_view,
                                      &session::user_name_view>,
                        const_mem_fun<session, std::experimental::string_view,
                                      &session::script_name_view>>,
          composite_key_hash<std::hash<std::experimental::string_view>,
...
void function_index() {
  boost::multi_index_container<
      session,
      indexed_by<hashed_unique<
          tag<by_name>, const_mem_fun<session, std::string, &session::id>>>>
      sessions;
