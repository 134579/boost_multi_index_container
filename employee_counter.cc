...
struct employee {
  ...
  std::experimental::string_view first_name_view() const {
    return {first_name.get().c_str(), first_name.get().size()};
  }
...
void simple_index() {
  boost::multi_index_container<
      employee, indexed_by<hashed_unique<
                    tag<by_name>,
                    const_mem_fun<employee, std::experimental::string_view,
                                  &employee::first_name_view>,
