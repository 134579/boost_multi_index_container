...
struct stock {
  stock(const std::string &_market_ref, const std::string &_id, double _price,
        int _volume)
      : market_ref(_market_ref),
        market_ref_view(_market_ref.data(), _market_ref.size()), id(_id),
        price(_price), volume(_volume) {}

  std::experimental::string_view get_market_ref_view() const {
    return market_ref;
  }
...
private:
  ...
  boost::multi_index_container<
      stock,
      indexed_by<hashed_unique<tag<by_reference>,
...
private:
  ...
  boost::multi_index_container<
      stock, indexed_by<hashed_unique<
                 tag<by_reference_view>,
                 const_mem_fun<stock, std::experimental::string_view,
                               &stock::get_market_ref_view>,
...
struct market_data_provider_umap_string {
  ...
  void add_stock(const stock &s) { m_stocks.emplace(s.market_ref, s); }
