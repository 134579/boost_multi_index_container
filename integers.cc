...
int main() {
  ...
  volatile int x =
      0; // its only reason is to avoid the compiler to optimize lookups

  {
    boost::multi_index_container<
        A, indexed_by<
               ordered_unique<tag<tags::x_asc>, member<A, int, &A::x>>,
               ordered_unique<tag<tags::y_asc>, member<A, int, &A::y>>,
               hashed_unique<tag<tags::unordered>, identity<A>, std::hash<A>>>>
        mic;

    benchmark([&]() { mic.emplace(rng(gen), rng(gen)); }, "boost.mic insert");

    auto &&h = mic.get<tags::unordered>();
    benchmark([&]() { x += h.find(A(rng(gen), rng(gen))) != h.end(); },
              "boost.mic lookup");
