...
template <typename T> struct tracker {
  tracker() { s_objects.insert(this); }
  tracker(const tracker &) { s_objects.insert(this); }
  tracker(tracker &&) { s_objects.insert(this); }
  ...
  static void print_instances() {
    std::cout << s_objects.size() << " " << typeid(T).name()
              << " instances:" << std::endl;
...
template <typename T> std::unordered_set<void *> tracker<T>::s_objects;

template <typename Tag> struct key : tracker<key<Tag>> {
...
namespace std {
// template <>
template <typename Tag> struct hash<key<Tag>> {
...
struct A : tracker<A> {
  A(int i, int j) : k(i), k2(i), v(j), _i(i) {}
...
int main() {
  boost::multi_index_container<
      A, indexed_by<hashed_unique<member<A, key<tags::k1>, &A::k>,
                                  std::hash<key<tags::k1>>>,
                    hashed_non_unique<member<A, key<tags::k2>, &A::k2>,
                                      std::hash<key<tags::k2>>>,
                    ordered_unique<member<A, int, &A::_i>>, random_access<>>>
      m;
